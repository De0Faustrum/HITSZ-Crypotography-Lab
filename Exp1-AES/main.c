 ...
 */
int mergeArrayToInt(const int array[4]){
    int one = array[0] << 24;
    int two = array[1] << 16;
    int three = array[2] << 8;
    int four = array[3];
...
void mixColumns(int array[4][4]){
    ...
    for(int i=0; i<4; i++){
        for (int j=0; j<4; j++){
            array[i][j] = GFMul(colM[i][0],temp[0][j]) ^ GFMul(colM[i][1],temp[1][j]) ^ GFMul(colM[i][2],temp[2][j]) ^ GFMul(colM[i][3], temp[3][j]);
        }
    }
}
...
void deMixColumns(int array[4][4]){
    ...
    for(int i=0; i<4 ; i++){
        for (int j=0; j<4 ; j++){
            array[i][j] = GFMul(deColM[i][0],temp[0][j]) ^ GFMul(deColM[i][1],temp[1][j]) ^ GFMul(deColM[i][2],temp[2][j]) ^ GFMul(deColM[i][3], temp[3][j]);
        }
    }
}
...
void addRoundKey(int array[4][4], int round){
    ...
    int roundKey[4];
    for (int i=0; i<4; i++){
        splitIntToArray(w[round*4+i], roundKey);
        for(int j=0; j<4; j++){
            array[j][i] = array[j][i] ^ roundKey[j];
        }
    }
}
...
int T(int num, int round){
    //请补充代码
    int splitArray[4] = {0, 0, 0, 0};
    splitIntToArray(num, splitArray); // num转为4位
    leftLoop4int(splitArray, 1); // 循环左移一位
    for(int i=0; i<4; i++){
        splitArray[i] = getNumFromSBox(splitArray[i]);
    }
    return mergeArrayToInt(splitArray) ^ (int)Rcon[round];
}

void getMatrixFromInt(int i, int array[4][4]){
    int index = i * 4;
    int transposedMatrix[4][4];
    splitIntToArray(w[index+0], transposedMatrix[0]);
    splitIntToArray(w[index+1], transposedMatrix[1]);
    splitIntToArray(w[index+2], transposedMatrix[2]);
    splitIntToArray(w[index+3], transposedMatrix[3]);
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            array[i][j] = transposedMatrix[j][i];
        }
    }
}

void oplusArray(int aArray[4][4], int bArray[4][4]){
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
...
void deAes(char *c, int clen, char *key){
    ...
    for(k = 0; k < clen; k += 16)    {
        ...
        for(int i = 9; i >= 1; i--)        {
            ...
            deMixColumns(cArray);
            getMatrixFromInt(i, wArray);
            deMixColumns(wArray);

            oplusArray(cArray, wArray);
        }
